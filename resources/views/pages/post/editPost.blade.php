                    ...
                    <form class="space-y-8" method="POST" action="{{ route('posts.update', $post->id) }}">
                        ...
                        <div>
                            ...
                            <input type="text" id="titulo" name="titulo" value="{{ old('titulo', $post->titulo) }}"
                                class="w-full px-4 py-3 border bg-white border-stone-300 rounded-md focus:ring-2 focus:ring-amber-200 focus:border-amber-400"
                            ...
                            <div>
                                ...
                                <div class="flex items-center gap-4 p-4 bg-stone-50 rounded-md border border-stone-200">
                                    <div class="w-20 h-20 rounded-md overflow-hidden">
                                        <img src="{{ $post->image }}" alt="Imagen actual"
                                            class="w-full h-full object-cover">
                                    </div>
                                    <div class="flex-1">
                                        <p class="font-medium text-stone-800">Imagen actual del post</p>
                                        <p class="text-sm text-stone-500">Actualizada:
                        ...
                        <div>
                            ...
                            <input type="url" id="image" name="image" value="{{ old('image', $post->image) }}"
                                class="w-full px-4 py-3 border bg-white border-stone-300 rounded-md focus:ring-2 focus:ring-blue-200 focus:border-blue-300"
                        ...
                        <div>
                            ...
                            <select name="category" id="category" required
                                ...
                                <option value="living"
                                    {{ old('category', $post->categoria->nombre ?? '') == 'living' ? 'selected' : '' }}>
                                    Living</option>
                                <option value="cocina"
                                    {{ old('category', $post->categoria->nombre ?? '') == 'cocina' ? 'selected' : '' }}>
                                    Cocina</option>
                                <option value="baño"
                                    {{ old('category', $post->categoria->nombre ?? '') == 'baño' ? 'selected' : '' }}>Baño
                                </option>
                                <option value="dormitorio"
                                    {{ old('category', $post->categoria->nombre ?? '') == 'dormitorio' ? 'selected' : '' }}>
                                    Dormitorio</option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea id="contenido" name="contenido" rows="20" required
                                class="w-full px-4 py-4 border bg-white border-stone-300 rounded-md focus:ring-2 focus:ring-amber-200 focus:border-amber-400"
                        ...
                        <div class="flex items-center justify-between pt-6 border-t border-stone-200">
                            <a href="{{ route('posts.list') }}"
                                class="px-4 py-2 text-white hover:bg-[#cfbdaa] transition-colors  bg-[#918477] rounded-md">
                                Cancelar
                            </a>
